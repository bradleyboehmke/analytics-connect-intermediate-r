...
rescale <- function(x, digits = 2, na.rm = TRUE){
  ...
  # add if statements to ensure argument inputs are valid

  
  # original function computation
  if(isTRUE(na.rm)) x <- x[!is.na(x)]
  rng <- range(x)
  scaled <- (x - rng[1]) / (rng[2] - rng[1])
  round(scaled, digits = digits)
...
# variance equation: (1 / (length(x) - 1)) * sum((x - mean(x))^2)
...
# standard deviation equation: sqrt((1 / (length(x) - 1)) * sum((x - mean(x))^2))
...
# standard error can be computed with the following sequence:
...
# skewness can be computed with the following sequence:
n <- length(x)
v <- var(x)
m <- mean(x)
